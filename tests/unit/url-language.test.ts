...
describe("language mapping", () => {
  ...
  it("supports broad repository languages and language-changing renames", () => {
    expect(detectLanguage("main.go")).toBe("go");
    expect(detectLanguage("policy.rego")).toBe("rego");
    expect(validateRenameLanguage("main.ts", "main.py")).toBe("python");
  });
});
