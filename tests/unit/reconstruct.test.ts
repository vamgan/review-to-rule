...
import { RefusalError } from "../../src/domain/errors.js";
import {
...
describe("deterministic reconstruction", () => {
  ...
  it("refuses unchanged, deletion, missing history, and oversize while preserving cross-language renames", () => {
    expect(() =>
      reconstruct(
        input({
          after: {
    ...
    expect(renamed.candidate.language).toBe("python");
    expect(renamed.evidence.rename).toEqual({
      from: "src/a.ts",
      to: "src/a.py",
    });
  });
