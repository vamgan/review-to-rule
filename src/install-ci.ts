...
export const CI_WORKFLOW = `name: review-to-rule\non:\n  pull_request:\n  push:\n    branches: [main]\npermissions:\n  contents: read\njobs:\n  semgrep-guardrails:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: 24\n      - uses: actions/setup-python@v5\n        with:\n          python-version: "3.12"\n      - run: pip install semgrep==1.175.0\n      - run: npm install --global review-to-rule@${GENERATOR_VERSION} --ignore-scripts\n      - run: review-to-rule validate-all .review-to-rule --repo-dir . --json\n      - run: semgrep scan --config .review-to-rule/rules --severity ERROR --error .\n`;
